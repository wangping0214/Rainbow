...
public class WebAction
{
	...
	/**
	 * 前台根据分类显示游戏和应用 1"创意休闲",2"飞行射击",3"体育竞技",4"益智棋牌",5"策略经营",6"竞速动作",7"角色扮演"
	...
	public String webShowByCategory()
	{
		...
		case 1:
			str = "创意休闲";
			clf = new Classification(1, 1, 2, 3, 4, "卡通", "连连看", "捕鱼", "祖玛",
					"创意休闲");
			break;
		case 2:
			str = "飞行射击";
			clf = new Classification(2, 5, 6, 7, 8, "射击", "阻击", "坦克", "飞行",
					"飞行射击");
			break;
		case 3:
			str = "体育竞技";
			clf = new Classification(3, 9, 10, 11, 12, "街机", "足球", "篮球", "台球",
					"体育竞技");
			break;
		case 4:
			str = "益智棋牌";
			clf = new Classification(4, 13, 14, 15, 16, "斗地主", "麻将", "纸牌",
					"拼图", "益智棋牌");
			break;
		case 5:
			str = "策略经营";
			clf = new Classification(5, 17, 18, 19, 20, "经营", "卡牌", "塔防", "战争",
					"策略经营");
			break;
		case 6:
			str = "竞速动作";
			clf = new Classification(6, 21, 22, 23, 24, "跑酷", "横版", "3D", "赛车",
					"竞速动作");
			break;
		case 7:
			str = "角色扮演";
			clf = new Classification(7, 25, 26, 27, 28, "回合制", "卡牌", "玄幻",
					"武侠", "角色扮演");
			break;
		case 8:
			str = "系统安全";
			clf = new Classification(8, 29, 30, 31, 32, "优化", "流量", "安全", "杀毒",
					"系统安全");
			break;
		case 9:
			str = "常用工具";
			clf = new Classification(9, 33, 34, 35, 36, "浏览器", "输入法", "文件",
					"办公", "常用工具");
			break;
		case 10:
			str = "摄影美化";
			clf = new Classification(10, 37, 38, 39, 40, "主题", "桌面", "壁纸",
					"相机", "摄影美化");
			break;
		case 11:
			str = "影音图像";
			clf = new Classification(11, 41, 42, 43, 44, "音乐", "视频", "相册",
					"播放器", "影音图像");
			break;
		case 12:
			str = "便捷生活";
			clf = new Classification(12, 45, 46, 47, 48, "健康", "购物", "理财",
					"旅行", "便捷生活");
			break;
		case 13:
			str = "社交通信";
			clf = new Classification(13, 49, 50, 51, 52, "聊天", "交友", "微博",
					"婚恋", "社交通信");
			break;
		case 14:
			str = "学习阅读";
			clf = new Classification(14, 51, 52, 53, 54, "电子书", "漫画", "新闻",
					"杂志", "学习阅读");
			break;
		default:
			break;
		...
		PageUtil page = new PageUtil(appTmp.getShowByCategoryCurrentPage(),
				total);
		page.setPageSize(16);
		List<AppInfo> appInfo = appInfoDAO.findByCategory(1, str,
				appTmp.getShowByCategoryCurrentPage(), page.getPageSize());
	...
	/**
	 * 根据具体类别显示游戏和应用 1 卡通,2 连连看，3 捕鱼，4 祖玛 5 射击，6 阻击 ，7 坦克， 8 飞行 9街机， 10足球， 11篮球，
	 * 12台球 13斗地主，14 麻将， 15纸牌， 16拼图 17经营 ，18卡牌， 19塔防，20 战争 21 跑酷， 22 横版， 23 3D，
	 * 24赛车 25回合制， 26卡牌 ，27玄幻， 28武侠 29优化， 30流量 ，31安全 ，32杀毒 33浏览器， 34输入法 ，35文件，
	 * 36办公 37主题，,38桌面， 39壁纸， 40相机 41音乐， 42视频 ，43相册 ，44播放器 45健康，46 购物， 47理财，
	 * 48旅行 49聊天， 50交友 ，51微博，52 婚恋 53电子书， 54漫画， 55新闻， 56杂志
	 * 
	 * @return
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public String webShowByClassification()
	{
		...
		PageUtil page = new PageUtil(
				appTmp.getShowByClassificationCurrentPage(), total);
		List<AppInfo> appInfo = appInfoDAO
				.findByClassification(1, str,
						appTmp.getShowByClassificationCurrentPage(),
						page.getPageSize());
		List<App> app = new ArrayList<App>();
		for (int i = 0; i < appInfo.size(); i++)
		{
		...
		List<AppInfo> topApp = appInfoDAO.findByCategory(1, clf.getStrCat(), 1,
				18);// 热门推荐
		List<App> topRecommended = new ArrayList<App>();
		for (int i = 0; i < topApp.size(); i++)
		{
	...
	public String showByRecomLevel()
	{
		...
		PageUtil page = new PageUtil(appTmp.getShowByRecomLevelCurrentPage(),
				total);
		page.setPageSize(15);
		List<AppAuthority> appAut = appAutDAO.findByRecomLevel(0,
				appTmp.getShowByRecomLevelCurrentPage(), page.getPageSize());
		List<App> app = new ArrayList<App>();
	...
	public String showByScoreAndDown()
	{
		...
		PageUtil page = new PageUtil(appTmp.getShowByScoreAndDownCurrentPage(),
				total);
		page.setPageSize(15);
		List<AppAuthority> appAut = appAutDAO.findByScoreAndDown(0,
				appTmp.getShowByScoreAndDownCurrentPage(), page.getPageSize());
		List<App> app = new ArrayList<App>();
		for (int i = 0; i < appAut.size(); i++)
		{
			AppAuthority authority = appAut.get(i);
	...
	/**
	 * 今日推荐 安卓精品 推荐级别 > 评分 > 下载量 柴米推荐->评分 人气排行榜->推荐级别
	 * 
	 * @return
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	...
	/**
	 * 安卓应用排行->应用->推荐级别 > 评分 > 下载量 > 发布时间 应用人气排行
	 * 
	 * 安卓游戏排行->游戏->推荐级别 > 评分 > 下载量 > 发布时间 游戏人气排行
	 * 
	 * @return
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	...
	public String rankMore()
	{
		...
		if (rankMoreType == 1)
		{
			...
			page.setPageSize(16);
			appAut = appAutDAO.findByTypeRank("应用", 0, 0, rankMoreCurrentPage,
					page.getPageSize());
	...
	public String rankByKeyword()
	{
		...
		int type = 1;
		String[] key = { "优化", "流量", "安全", "杀毒", "浏览器", "输入法", "文件", "办公",
				"主题", "桌面", "壁纸", "相机", "音乐", "视频", "相册", "播放器", "健康", "购物",
				"理财", "旅行", "交友", "聊天", "微博", "婚恋", "电子书", "漫画", "新闻", "杂志" };
		...
		if (type == 0)
			...
			return "rank_moreGame";

	}

	/**
	 * apk应用的具体信息 根据用户下载应用的类别classification。推荐其他应用
	 * 
	 * @return
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public String details()
	{
		...
		// 推荐其他应用
		List<AppAuthority> appAut = appAutDAO.findByClassification(
				info.getClassification(), apkId, 0, 0, 1, 5);
