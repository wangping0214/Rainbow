...
public class AdminAction
{
	...
	{
		Admin findAdmin = adminDAO.find("username", admin.getUsername(),
		...
		if (findAdmin != null)
		{
			...
			httpSession.setAttribute("admin", findAdmin);
	...
	public String information() throws UnsupportedEncodingException
	{
		...
		{
			User group_info = userDAO.find("username", memusername, "userType",
					getMemusertype());
			@SuppressWarnings("rawtypes")
			Map request = (Map) ActionContext.getContext().get("request");
		...
		{
			User user_info = userDAO.find("username", memusername, "userType",
					getMemusertype());
			@SuppressWarnings("rawtypes")
			Map request = (Map) ActionContext.getContext().get("request");
		...
		{
			User user = userDAO.find("username", memusername, "userType",
					getMemusertype());
			@SuppressWarnings("rawtypes")
			Map request = (Map) ActionContext.getContext().get("request");
	...
	public String reg_information() throws UnsupportedEncodingException
	{
		if (getMemusertype().equals("individualGroups"))
		{
			...
			httpSession.setAttribute("corporatename",
					group_info.getCorporatename());
	...
	public String reg_approved()
	{
		if (("ok").equals(getRadioselect()))
		{
			HttpSession httpSession = ServletActionContext.getRequest()
					.getSession();
			User reg_user = userDAO.find("username",
					(String) httpSession.getAttribute("reg_username"));
			if (reg_user != null)
			{
				reg_user.setApproved(1);// 审核通过
				/*
				 * 生成 商户ID 根据当前用户注册id生成一个长度为8位的商户id传入数据库中
				 */
				if (reg_user.getCp_id() == null)//新注册用户
				{
