					...
					<div class="b2">
						<s:iterator value="#request['app']" id="app" status="st">
							...
							<div class="every">
								<p class="num">
									<s:property
										value="#st.count + (#page.currentPage-1)*#page.pageSize" />
								</p>
								<dl>
									<dt>
										<a
											href="showInfo.action?appTmp.showInfoId=<s:property value='#app.appAut.id' />"
											target="main"><img
									...
									<dd>
										<a
											href="showInfo.action?appTmp.showInfoId=<s:property value='#app.appAut.id' />"><s:property
												value="#app.appInfo.appName" /></a>
									</dd>
								</dl>
								<div class="shuju">
									<div>
										<label>推荐级别选择</label><input name="appAut.recomLevel"
											id="recomLevel<s:property value='#st.count' />" type="text"
											value="<s:property value="#app.appAut.recomLevel" />" />
										<p>取值0-100</p>
									</div>
									<div>
										<label>评分</label><input name="appAut.score"
											id="score<s:property value='#st.count' />" type="text"
											value="<s:property value="#app.appAut.score" />" />
										<p>取值0-100</p>
									</div>
									<div>
										<label>下载量</label><input name="appAut.amountOfDown"
											id="amountOfDown<s:property value='#st.count' />" type="text"
											value="<s:property value="#app.appAut.amountOfDown" />" /><span>次</span>
									</div>
									<div class="sure">
										<input type="submit" value="确认修改"
											onclick="rankSubmit(<s:property value='#app.appAut.id' />,<s:property value='#st.count' />)" />
									</div>
								</div>
							</div>
						</s:iterator>
						...
						<ul class="page">
							...
							<s:if test="#page.hasFirst">
								<li><a href="rank.action?appTmp.rankCurrentPage=1">首页</a></li>
							</s:if>
							...
							<s:if test="#page.hasLast">
								<li><a
									href="rank.action?appTmp.rankCurrentPage=<s:property value="#page.totalPage"/>">尾页</a></li>
							</s:if>
