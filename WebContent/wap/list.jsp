<%@ page language="java" import="com.rainbow.entity.Prize" import="java.util.List;" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
	<%List<Prize> prize = (List)session.getAttribute("prize"); %>
...
<p class="atit">本期获奖用户：<%=prize.size() %></p>
<div id="demo">
    <div id="demo1">
    	<%if(prize!=null){ 
    	for(int i=0;i<prize.size();i++){%>
        <div><p class="ming"><span><%=prize.get(i).getName().substring(0, 1) %>**</span>
        <span><%=prize.get(i).getPhone().substring(0, 3) %>****<%=prize.get(i).getPhone().substring(prize.get(i).getPhone().length()-5, prize.get(i).getPhone().length()-1) %></span></p>
        <p class="jiang"><%if(prize.get(i).getPrize()==1) {%>获得了机械游戏键盘一个<%} %>
