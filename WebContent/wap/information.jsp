...
$(document).ready(function(){
	$("#submit").click(function(){
		...
		var union = phone.substr(0,3);
		var isUnionPhone = false;

		if(union=="186"||union=="185"||union=="156"||union=="155"||union=="132"||union=="131"||union=="130"){
			isUnionPhone = true;
		}
		if(isUnionPhone == false){
			alert("对不起，您不是联通用户，不能参加抽奖活动！");
			return false;
		}
		var id_card_reg = /^(\d{6})(18|19|20)?(\d{2})([01]\d)([0123]\d)(\d{3})(\d|X)?$/;
		...
		jQuery.ajax({
			type:"post",
			url:"prizeCheckPhone.action",
			data:{"phone":phone},
			success:function(data){
				if(data=="1"){
					jQuery.ajax({
						type:"post",
						url:"prizeCheckIdCard.action",
...
<div class="infor">
   ...
   <div class="incont">
       ...
       <div><label><span><b>*</b>性别：</span><select name="prize.sex" id="sex"><option value="男">男</option><option value="女">女</option></select></label></div>
       <div><label><span><b>*</b>手机号码：</span><input type="text" name="prize.phone" id="phone"/></label></div>
       <div><label><span><b>*</b>身份证号码：</span><input type="text" name="prize.id_card" id="id_card"/></label></div>
       <div><label><span><b>*</b>省份：</span><select id="id_addr1" name="selectp" onchange="selectcityarea('selectp','selectc','form');"><option value="0" selected="selected">--不限--</option></select></label></div>
       <div><label><span><b>*</b>城市：</span><select id="id_addr2" name="selectc"><option value="0" selected="selected">--不限--</option></select></label></div>
       <div><label><span><b>*</b>详细地址：</span><input type="text" name="prize.address" id="address"/></label></div>
       <div><label><span><b>*</b>邮编：</span><input type="text" name="prize.zip_code" id="zip_code"/></label></div>
   </div>
   </form>
   <div class="tijiao"><input type="button" id="submit" value="提交" /></div>
