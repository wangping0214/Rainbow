	...
	<div align="center">
		<div align="left">
			...
			<select id="status">
				...
				<option <%if ("01".equals(month)) {%> selected="selected" <%}%>
					value="1">1月</option>
				<option <%if ("02".equals(month)) {%> selected="selected" <%}%>
					value="2">2月</option>
				<option <%if ("03".equals(month)) {%> selected="selected" <%}%>
					value="3">3月</option>
				<option <%if ("04".equals(month)) {%> selected="selected" <%}%>
					value="4">4月</option>
				<option <%if ("05".equals(month)) {%> selected="selected" <%}%>
					value="5">5月</option>
				<option <%if ("06".equals(month)) {%> selected="selected" <%}%>
					value="6">6月</option>
				<option <%if ("07".equals(month)) {%> selected="selected" <%}%>
					value="7">7月</option>
				<option <%if ("08".equals(month)) {%> selected="selected" <%}%>
					value="8">8月</option>
				<option <%if ("09".equals(month)) {%> selected="selected" <%}%>
					value="9">9月</option>
				<option <%if ("10".equals(month)) {%> selected="selected" <%}%>
					value="10">10月</option>
				<option <%if ("11".equals(month)) {%> selected="selected" <%}%>
					value="11">11月</option>
				<option <%if ("12".equals(month)) {%> selected="selected" <%}%>
					value="12">12月</option>
			</select> <input type="button" value="查询" onclick="initFinancial(1)" />
			<%
				int i = 0;
			...
			<div>
				...
				<table>
					...
					%>
					<tr>
						<td><input type="button" id="check<%=i %>"
							...
							<%if (financial.getCheck_out() == 0) {%> value="未结算"
					...
					%>
					<tr>
						<td><input type="button" value="未结算" id="check<%=i %>"
						onclick="checkOutAppFinancial(<%=i %>,<%=financial.getApp().getAppInfo().getId()%>,<%=financial.getApp().getAppInfo().getAppName()%>,<%=financial.getPaySum()%>)"
						/></td>
						<td><%=financial.getApp().getAppInfo().getAppName()%></td>
						<td><%=financial.getSum()%>元</td>
					...
					%>
					<tr>
						<td><input type="button" value="已结算" id="check<%=i %>" disabled="disabled" /></td>
						<td><%=financial.getApp().getAppInfo().getAppName()%></td>
						<td><%=financial.getSum()%>元</td>
						<td><%=financial.getMassageSum()%>元</td>
						<td><%=financial.getAlipaySum()%>元</td>
